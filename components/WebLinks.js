...
const Links = () => {
  ...
  // Collect all links filter by type - social, project, nft and market etc=
  // get data for social section
  const social = allLinks.filter((el) => {
    return el.type === "social" && el.on
  });

  // Get data for market section
  const market = allLinks.filter((el) => {
    return el.type === "mds" && el.on
  });
  ...
  // Get data for market section
  const markets = allLinks.filter((el) => {
    return el.type === "market" && el.on
  });
